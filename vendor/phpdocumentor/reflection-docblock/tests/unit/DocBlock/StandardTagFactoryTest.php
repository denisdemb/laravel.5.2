...
class StandardTagFactoryTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $context    = new Context('');
        $tagFactory = new StandardTagFactory(m::mock(FqsenResolver::class), ['users' => Author::class]);

        /** @var Author $tag */
        $tag = $tagFactory->create('@users Mike van Riel <me@mikevanriel.com>', $context);
     ...
     * @expectedExceptionMessage The tag "@users/myuser" does not seem to be wellformed, please check it for errors
     */
    public function testExceptionIsThrownIfProvidedTagIsNotWellformed()
    {
        ...
        ;
        $tagFactory = new StandardTagFactory(m::mock(FqsenResolver::class));
        $tagFactory->create('@users[myuser');
    }
